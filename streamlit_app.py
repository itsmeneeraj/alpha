...
st.set_page_config(page_title="Goal Based Wealth Management", page_icon="💸", layout="wide")
...
# User inputs
upload_option = st.radio("Choose an option:", ("Update Details Manually", "Upload Excel File"))
...
if upload_option == "Update Details Manually":
    with st.form(key='investment_parameters'):
...
if st.session_state.submitted:
    if st.session_state.upload_option == "Update Details Manually":
        for i in range(st.session_state.num_goals):
            Goal = st.number_input(f"Goal Amount for Goal {i + 1}", value=float(st.session_state.goals[i][0]), step=1000.0, key=f'goal_{i}')
            Years = st.number_input(f"Number of Years for Goal {i + 1}", value=float(st.session_state.goals[i][1]), step=1.0, key=f'years_{i}')
            Priority = st.number_input(f"Priority for Goal {i + 1} (1-10)", value=int(st.session_state.goals[i][2]), min_value=1, max_value=10, step=1, key=f'priority_{i}')
    ...
    if st.button("Calculate Investments"):
        ...
        for i, (Goal, Years, Priority) in enumerate(st.session_state.goals):
            ...
            if remaining_investment_capacity > 0:
                original_solution = goal_programming_solver(RE, RD, Years, p_e, Goal)
                remaining_investment_capacity -= original_solution
                st.success(f"Minimum investment needed to achieve goal amount {Goal} in {Years} years with priority {Priority}: \u20B9{original_solution:.2f}")
